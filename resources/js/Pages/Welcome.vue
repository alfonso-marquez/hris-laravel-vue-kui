...
<template>
    ...
    <div
        class="relative flex items-top justify-center min-h-screen bg-gray-100 dark:bg-dark-eval-0 dark:text-gray-200 items-center sm:pt-0">
        <div v-if="canLogin" class="fixed flex items-center top-0 right-0 px-6 py-4 sm:block">
            <span class="flex items-center">
                <Button iconOnly variant="secondary" type="button" @click="toggleDarkMode" v-slot="{ iconSizeClasses }"
                    class="inline-flex mr-5" srText="Toggle dark mode">
                ...
                <Link v-if="$page.props.auth.user" href="/dashboard"
                    class="inline-flex text-sm text-gray-700 hover:underline">Dashboard
                </Link>
                <Link v-if="$page.props.auth.user" :href="route('logout')"
                    class="inline-flex text-sm text-gray-700 hover:underline ml-4" method="post" as="button">Logout
                </Link>
                <template v-else>
                    <Link :href="route('login')" class="text-sm text-gray-700 hover:underline">Log in</Link>
                    <Link v-if="canRegister" :href="route('register')"
                        class="ml-4 text-sm text-gray-700 hover:underline">
                    Register
                    </Link>
                </template>
            </span>
